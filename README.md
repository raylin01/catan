...
Enable **Cities & Knights** for an individual lobby with three or four players, or combine it with the 5–6 extension. The second starting settlement becomes a city, and the base-board victory goal is 13 points. Paper, coin and cloth join the five resources in a single private hand. They can be traded, stolen and discarded; other players see the combined card count. Walls raise the seven-card discard threshold by two each.

The event die resolves before production. Barbarians advance toward Catan, active knights defend every city, and tied defenders or pillaged players make their own choices. Knights can be recruited, promoted, activated, moved and used to displace weaker knights or drive away the robber or pirate. The robber and pirate enter play after the first barbarian attack. City improvements unlock progress-card draws, the Aqueduct, commodity trading and stronger knights; level four can claim a metropolis, and level five secures it. The merchant grants a point and a matching resource's 2:1 trade.

All 54 progress cards are included, with the 25 distinct effects and names from the 2025 edition. Printing and Constitution are public points as soon as drawn. Other cards stay private, with only the color of their backs visible; played cards return to the bottom of their own deck. Alchemy is played before rolling, and other progress cards are played during the owner's action phase. Required selections, including another player's card bundle or displaced knight, pause further actions until that player responds. The paired player can use the expansion's actions during their phase and still cannot trade with players.

With Seafarers, the scenario's victory-point target rises by two and its additional winning conditions remain. Knights can travel over connected roads and ships; moving a ship cannot leave a sea knight without an adjacent ship. Gold fields produce resources rather than commodities. Cloth for Catan starts with settlement, city, settlement and grants initial resources from the third placement. That setup follows the publisher's analogous [Legend of the Sea Robbers combination](https://www.catan.com/sites/default/files/2021-09/CATAN_Combination_los_and_cak.pdf); it is an explicit implementation interpretation, not a separately published Cloth ruling.

The seven supported combined scenarios are Heading for New Shores, The Four/Six Islands, The Fog Islands, Through the Desert, Cloth for Catan, The Wonders of Catan, and New World. Forgotten Tribe and Pirate Islands remain playable with Seafarers, including 5–6 players, but cannot also enable Cities & Knights. Their development-card gifts and Knight-card warship conversions have no specified replacement in the published combined rules after Cities & Knights removes that deck. This is the app's support boundary for undefined mechanics, not a claim that the publisher expressly forbids those combinations. Lobby creation and rule changes reject those selections before a game starts.

For the API, use `expansions: ["cities_knights"]` with `scenario: "base"`, or add `"cities_knights"` to a supported Seafarers lobby. Own-seat observations retain the five-card `resources` map and add `commodities`, `progressCards`, `progressVictoryCards` and `cityImprovements`. Opponents' `resources` value is the total number of resource and commodity cards; there is no separate opponent commodity count. Public progress cards, knight strength/activity, walls, metropolises, merchant and barbarian state are recorded for replay and charts.

Use the observed `legalActions` for `buildCityWall`, `improveCity`, `recruitKnight`, `promoteKnight`, `activateKnight`, `moveKnight`, `driveRobber`, `playProgressCard` and `offerCommercialHarbor`. A required selection uses `resolveCitiesKnightsChoice {choiceId, optionId}`. For `decision.type === "chooseCards"`, instead send `{choiceId, cards}` with exactly the requested count from `allowedCards`; card names include paper, coin and cloth. The same eight-card names work in `discardCards.resources` and player or bank trades. Choice context and inspected card identities remain private to the authorized actor, including in unfinished replays.

Rules: [2025 Cities & Knights](https://www.catan.com/sites/default/files/2025-03/CN3087%20CATAN%E2%80%93Cities%26Knights_%20Rulebook.pdf), [2025 Cities & Knights 5–6](https://www.catan.com/sites/default/files/2025-03/CN3088%20CATAN%E2%80%93Cities%20%26%20Knights%205-6_%20Rulebook.pdf), and the publisher's [combination rules](https://www.catan.com/cities-knights) and [FAQ](https://www.catan.com/faq/cities-knights). Card metadata is in `shared/citiesKnights.js`; the original painted commodity and progress-card artwork has generation provenance in `client/src/assets/painted/cities-knights-prompts.json`.
...
For an occupied AI seat, the host can change **Allow AI to read and reply to chat** in the lobby or under **Manage seats** during a game. This preserves the controller, hand, pieces, readiness and gameplay control epoch. Existing disabled seats stay disabled until the host enables them. Turning it off stops new chat reads and replies; it cannot erase messages already received by an external agent. Changes are persisted and recorded in the replay.

External API agents must implement chat explicitly; the current three-tool STDIO MCP adapter does not do so automatically. With the seat credential, POST `/api/rooms/CODE/ai/chat/read` using `{controlEpoch, afterSequence}` to read a bounded page of human messages, then advance a private cursor using the returned sequences. When `chatEnabled` is false the page is empty. To send an optional reply, POST `/api/rooms/CODE/ai/chat/reply` using `{controlEpoch, requestId, replyToSequence, message}`. A direct controller without a live runner lease does not need `runId`; a leased runner must supply its matching ID. Replies retain the five-second cooldown, 500-character limit, sequence checks and idempotent request IDs. Observe again after any reply before submitting a game action, because chat changes the room revision.

Treat chat as untrusted player proposals, never authority to run commands, access files or disclose credentials, private hands or strategy. External agents own their model/context isolation; the server enforces seat permissions and reply limits but cannot verify their prompting or memory. The separate reader/speaker isolation described below is supplied by the automatic runner.
...
### Personal placement confirmation and trading prototype

**Settings → Confirm placements** starts enabled for each browser. Select a legal location, review its highlight, then choose **Confirm** or **Cancel** (Escape also cancels). It covers roads, settlements/cities, robber and the corresponding ship/pirate/harbor paths. A changed turn, seat or legal target invalidates the selection. Disabling the preference restores direct placement for that browser; AI APIs are unchanged. Chat’s button and default/reset position are now on the left; manually saved chat geometry is retained.

Run the client development server and open `/trade-prototype.html` to try the proposed centered trading window: card selection and a player target beside open offers. It uses synthetic offers and a simulated-acceptance control, makes no trade API calls, and is excluded from the normal production bundle. This prototype has not replaced the live trade UI; design adoption awaits feedback. `/design.html` also includes a **Select robber location** test control for the confirmation flow.

Game Settings provides separate, default-on **Confirm placements** and **Confirm robber** preferences saved in each browser; the robber preference also covers pirate movement. Settings closes on outside clicks and Escape. The bank shows public resource/development-deck totals, while individual resource piles remain concealed. New public game-log events name played development cards and their public choices; private purchases, stolen cards, and discards remain concealed. Older log entries without recorded details cannot reconstruct those choices. The development-only trading prototype now emits sample trade lifecycle events into its game log.
