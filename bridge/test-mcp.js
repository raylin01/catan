...
function startMcp(server,sessionPath) {
  const child=spawn(process.execPath,[join(here,'cli.js'),'mcp','--server',server,'--session',sessionPath],{
    cwd:dirname(here),stdio:['pipe','pipe','pipe'],
  });
...
test('MCP stdio joins, observes, acts, and loses access when its seat is revoked',{timeout:15000},async()=>{
  ...
  const created=service.create({name:'Host',seatCount:3,seats:[{kind:'ai',provider:'codex'},{kind:'human'},{kind:'human'}]});
  ...
  try {
    ...
    const joinedReply=await mcp.request('tools/call',{name:'catan_join',arguments:{code:created.code,name:'MCP Bot',provider:'codex'}});
