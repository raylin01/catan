...
export function decisionSchemaFor(view) {
  ...
  let operations=view.gameState?.turnPhase!=='main'||view.gameState?.playerTradingAllowed===false?[]:!trade?['tradeOffer']
...
export function modelObservation(view) {
  ...
  const observation=Object.fromEntries(['seatId','generation','revision','gameOptions','gameState','legalActions','decision','trade','robberPick','proposals'].filter(key=>view[key]!==undefined).map(key=>[key,view[key]]));
  if(view.negotiation) {
