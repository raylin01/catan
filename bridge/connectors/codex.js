...
export function decisionSchemaFor(view) {
  ...
  const operations=view.gameState?.turnPhase!=='main'?[]:!trade?['tradeOffer']
  ...
  if(operations.length)schema.properties.trade.anyOf[0].properties.operation.enum=operations;
...
return {action,memory:result.memory,contextId:completed.contextId,publicReply:result.publicReply};
}
