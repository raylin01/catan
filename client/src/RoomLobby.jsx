...
import NewWorldPreview from './components/NewWorldPreview';
import GameRulesFields, {gameOptionsForDraft, gameRulesLabel, rulesDraft, validRulesDraft} from './components/GameRulesFields';
...
function RoomLobby({
  ...
  const [createRules, setCreateRules] = useState(() => rulesDraft());
  const [roomRules, setRoomRules] = useState(() => rulesDraft());
  ...
  const savedOptions = JSON.stringify(snapshot?.gameOptions || {version: 1, extension56: false, expansions: [], scenario: 'base'});
  const rulesChanged = roomRules.seatCount !== String(roomSeatCount)
    || JSON.stringify(gameOptionsForDraft(roomRules)) !== JSON.stringify(gameOptionsForDraft(rulesDraft(JSON.parse(savedOptions), roomSeatCount)));

  useEffect(() => {
    if (!snapshot) return;
    setRoomRules(rulesDraft(JSON.parse(savedOptions), roomSeatCount));
  }, [snapshot?.code, roomSeatCount, savedOptions]);
  ...
  const submitCreate = async event => {
    ...
    await onCreateRoom({
      name: name.trim(),
      seatCount: Number(createRules.seatCount),
      seats: Array.from({ length: Number(createRules.seatCount) }, () => ({ kind: 'human' })),
      gameOptions: gameOptionsForDraft(createRules),
    ...
    return (
      <main className="room-lobby room-lobby-room">
        <div className="room-shell">
          ...
          {!ended && <><section className="room-section room-rules-section" aria-labelledby="room-rules-heading">
            <div className="room-section-heading">
              <div>
                <h2 id="room-rules-heading">Game rules</h2>
                <p>{gameRulesLabel(snapshot.gameOptions, roomSeatCount)}</p>
                {snapshot.gameOptions?.setup && <p>{snapshot.gameOptions.setup.layout === 'variable' ? 'Variable layout' : 'Official fixed layout'}{snapshot.gameOptions.setup.seed != null ? ` · Seed ${snapshot.gameOptions.setup.seed}` : ''}</p>}
              </div>
            </div>
            {isHost && (
              <form className="room-rules-controls" onSubmit={event => { event.preventDefault(); onHostCommand('configureGame', {
                  seatCount: Number(roomRules.seatCount),
                  gameOptions: gameOptionsForDraft(roomRules)
                }); }}>
                <GameRulesFields value={roomRules} onChange={setRoomRules} disabled={busy}/>
                <button type="submit" className="room-secondary-button" disabled={!rulesChanged || !validRulesDraft(roomRules) || busy}>Save rules</button>
                <p className="room-field-help">Changing rules clears every ready status.</p>
              </form>
            )}
            {snapshot.boardPreview && <NewWorldPreview preview={snapshot.boardPreview} gameOptions={snapshot.gameOptions} seatCount={roomSeatCount} isHost={isHost} busy={busy} rulesChanged={rulesChanged} onSave={gameOptions=>onHostCommand('configureGame',{seatCount:roomSeatCount,gameOptions})}/>}
          </section>
      ...
      <div className="room-shell room-home-shell">
        ...
        {mode === 'create' && (
          <form className="room-form" onSubmit={submitCreate}>
            ...
            <GameRulesFields value={createRules} onChange={setCreateRules} disabled={busy}/>
            ...
            <button type="submit" className="room-primary-button" disabled={busy || !validRulesDraft(createRules) || (site?.available === 0 && !hostKey.trim())}>
