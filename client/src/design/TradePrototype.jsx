...
export function TradePrototypeProvider({children}){const [trades,setTrades]=useState(seed);return <Context.Provider value={{trades,setTrades}}>{children}</Context.Provider>;}
export function TradePrototypeTools(){const {setTrades}=useContext(Context);return <aside className="trade-prototype-tools"><span><strong>Trading prototype</strong> · Sample offers, no live game</span><div><button onClick={()=>setTrades(ts=>ts.map(t=>t.from==='seat-a'?{...t,status:'accepted'}:t))}>Simulate acceptance</button><button onClick={()=>setTrades(seed())}>Reset offers</button></div></aside>;}
...
function PlayerTradePrototype({gameState,seatId,onClose}){
  const {trades,setTrades}=useContext(Context),players=gameState.players,me=players.find(p=>p.id===seatId),partners=players.filter(p=>p.id!==seatId);
  ...
  useEffect(()=>{if(counterId&&!trades.some(t=>t.id===counterId)){setCounterId(null);setGive({});setGet({});setNotice('That offer is no longer open. Start a new offer.');}},[trades,counterId]);
  const reset=()=>{setCounterId(null);setGive({});setGet({});};
  const update=(setter,c,n)=>{setter(v=>({...v,[c]:Math.max(0,n)}));setNotice('');};
  const change=(id,status)=>{setTrades(ts=>status?ts.map(t=>t.id===id?{...t,status}:t):ts.filter(t=>t.id!==id));setNotice(status?'Offer accepted. The other player confirms the exchange.':'Offer removed.');dialog.current?.focus();};
  const send=()=>{if(!valid)return;setTrades(ts=>{const previous=ts.find(t=>t.id===counterId);const history=previous?[...(previous.history||[]),{id:previous.id,from:previous.from,to:previous.to,give:previous.give,get:previous.get}]:[];return [...ts.filter(t=>t.id!==counterId),{id:crypto.randomUUID(),from:seatId,to:partner,give:g,get:r,status:'offered',counterOf:counterId,history}];});setNotice(`Offer sent to ${names(partner)}. It’s in Open offers.`);reset();};
