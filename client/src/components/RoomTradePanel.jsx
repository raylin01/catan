...
export default function RoomTradePanel({snapshot,gameState,seatId,onCommand,onClose,addNotification,mode='player'}) {
  ...
  const bankAllowed=canTrade&&active?.id===seatId&&player?.resources?.[bankGive]>=ratio&&bankGive!==bankGet&&(gameState.bank?.[bankGet]??0)>0;
  ...
  return <div className="modal-overlay exchange-overlay" onClick={onClose}><section ref={dialog} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="exchange-title" className="exchange-dialog" onClick={e=>e.stopPropagation()}>
    ...
    {bank? <form onSubmit={e=>{e.preventDefault();if(bankAllowed)send('bankTrade',{giveResource:bankGive,giveAmount:ratio,getResource:bankGet},`Traded ${ratio} ${bankGive} for 1 ${bankGet}.`,true);}}>
      ...
      <div className="bank-choice-section"><h3>You receive <strong>1</strong></h3><div className="bank-choices">{TYPES.map(r=><button type="button" key={r} aria-pressed={bankGet===r} disabled={r===bankGive||!(gameState.bank?.[r]>0)} onClick={()=>setBankGet(r)}><GameIcon name={r} size={42}/><span>{name(r)}</span><small>{gameState.bank?.[r]||0} in bank</small></button>)}</div></div>
      ...
      {trade&&<section className="pending-exchange"><p className="exchange-status">{trade.status==='accepted'?`${recipientName} accepted. ${offererName} must confirm.`:`${offererName} offers a trade to ${recipientName}.`}</p>
        ...
        <div className="exchange-buttons">{isRecipient&&trade.status==='offered'&&<><button className="room-primary-button" disabled={!canTrade||busy} onClick={()=>send('tradeAccept',{tradeId:trade.id},'Trade accepted.')}>Accept offer</button><button className="room-secondary-button" disabled={!canTrade||busy} onClick={()=>{setPartner(trade.from);setGive({...empty(),...trade.get});setGet({...empty(),...trade.give});setCountering(true);}}>Counteroffer</button><button className="room-secondary-button" disabled={!canTrade||busy} onClick={()=>send('tradeReject',{tradeId:trade.id},'Trade rejected.',true)}>Reject</button></>}
        {isOfferer&&<><button className="room-secondary-button" disabled={!canTrade||busy} onClick={()=>send('tradeCancel',{tradeId:trade.id},'Offer cancelled.',true)}>Cancel offer</button>{trade.status==='accepted'&&<button className="room-primary-button" disabled={!canTrade||busy} onClick={()=>send('tradeConfirm',{tradeId:trade.id},'Trade completed.',true)}>Confirm trade</button>}</>}</div>
      </section>}
      {(!trade||countering)&&player&&<form onSubmit={offer}><label className="exchange-partner">Trade with<select value={partner} onChange={e=>setPartner(e.target.value)} disabled={countering||busy}>{partners.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
        ...
        <footer className="exchange-footer"><p>{countering?'Your counteroffer replaces the current offer.':'Resources move only after acceptance and confirmation.'}</p><button type="submit" className="room-primary-button" disabled={!canTrade||!partners.length||busy}>{busy?'Sending…':countering?'Send counteroffer':'Send offer'}</button></footer>
      </form>}
