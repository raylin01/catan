...
export default function CardMovements({events=[],seatId}) {
  ...
  useEffect(()=>{
    ...
    const start=window.setTimeout(()=>{
      const flights=next.transfers.flatMap((t,transferIndex)=>{
        ...
        return Array.from({length:Math.min(t.count,3)},(_,i)=>({...t,from,to,key:`${transferIndex}-${i}`,offset:i*7,delay:i*75+transferIndex*45,badge:i===Math.min(t.count,3)-1}));
      });
      setActive({id:next.id,flights});
      setNotice({id:next.id,text:summary(next,seatId)});
      end=window.setTimeout(()=>{setActive(null);setQueue(q=>q.filter(e=>e.id!==next.id));},1150+Math.max(0,...flights.map(f=>f.delay)));
    },next.type==='rollDice'?900:120);
  ...
  return createPortal(<div className="card-movement-layer">
