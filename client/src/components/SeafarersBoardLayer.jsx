...
export default function SeafarersBoardLayer({state,pirate,players,bounds,legalActions,onAction,paused,selectedAction,pendingPlacement,pirateMoving=false,tokensMoving=false}) {
  ...
  return <g className="seafarers-board-layer">
    ...
    {pirateActions.map(action=>{const point=framePoint(action.payload.hexKey,bounds);const selected=pendingPlacement?.type==='movePirate'&&pendingPlacement.payload.hexKey===action.payload.hexKey;return <g key={action.payload.hexKey} transform={`translate(${point.x} ${point.y})`} className={`pirate-frame-target ${selected?'placement-selected':''}`} role="button" tabIndex={0} aria-pressed={selected} aria-label={`Move pirate to ${action.payload.hexKey.slice(6)} frame`} onClick={()=>onAction(action)} onKeyDown={event=>keyPress(event,()=>onAction(action))}><circle className="choice-aura" r="22"/><circle className="vertex-placeholder-halo" r="19"/><GameIconSymbol name="pirate" size={27} style={{color:'#f6e6b6'}}/></g>;})}
