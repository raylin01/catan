...
export class RoomService {
  ...
  create(options={}) {
    ...
    const room={code,recordingId:secret(),revision:0,name:cleanName(name),gameOptions:prepared.gameOptions,
      ...(prepared.boardPreview?{boardPreview:prepared.boardPreview}:{}),slots,members:{[hash(token)]:{role:'host',name:cleanName(name)}},game:null,trade:null,chat:[],chatSequence:0,receipts:{},cardEventSequence:0,cardEvents:[],paused:false,interrupted:false};
  ...
  observe(code,token) {
    ...
    return {success:true,code,name:room.name,replayId:room.recordingId,revision:room.revision,role:member.role,host:member.role==='host',seatId:member.seatId||null,generation:member.generation||0,
