...
test('public extension lobbies preserve capacity, watch privacy, and closed replay rules',()=>{
  const store=new RoomStore(':memory:'),clock={now:1000};
  try {
    const service=new RoomService({store,maxRooms:1,now:()=>clock.now});
    const host=service.create({name:'Public extension',seatCount:5,gameOptions:extended});
    assert.equal(host.success,true);
    assert.equal(service.create({name:'Capacity is reserved'}).statusCode,429);
    assert.equal(issue(service,host,host,'configureGame',{seatCount:6,gameOptions:extended}).success,true);
    const watched=service.watch(host.code);
    assert.deepEqual(watched.gameOptions,extended);assert.equal(watched.slots.length,6);
    assert.equal(Object.hasOwn(watched,'replayId'),false);
    assert.equal(Object.hasOwn(watched.slots[0],'controller'),false);
    const actors=occupy(service,host);
    assert.equal(issue(service,host,host,'start').success,true);
    assert.equal(service.watch(host.code).gameState.players.length,6);
    assert.equal(issue(service,host,actors[0],'closeRoom').statusCode,403);
    clock.now=2000;
    assert.equal(issue(service,host,host,'closeRoom').success,true);
    const closed=service.watch(host.code);
    assert.equal(closed.status,'closed');assert.equal(closed.replayId,host.replayId);
    assert.deepEqual(closed.gameOptions,extended);assert.deepEqual(closed.legalActions,[]);
    const recording=store.getRecording(host.replayId);
    assert.equal(recording.rulesVersion,'catan-base-56-2025-v1');assert.equal(recording.endedAt,clock.now);
    const replay=service.replay(host.replayId,{perspective:'omniscient'});
    assert.equal(replay.success,true);assert.deepEqual(replay.state.gameOptions,extended);
    assert.equal(replay.state.gameState.phase,'finished');
    const next=service.create({name:'Freed public capacity',seatCount:4});
    assert.equal(next.success,true);assert.deepEqual(service.watch(next.code).gameOptions,baseGameOptions());
  } finally {store.close();}
});
