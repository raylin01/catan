...
import {queueGoldClaims} from './seafarersCore.js';
...
test('closing a Seafarers room clears pending choices for public watch and terminal replay',()=>{
  for(const reason of ['manual','idle']) {
    const store=new RoomStore(':memory:'),clock={now:1000};
    try {
      const service=new RoomService({store,inactivityMs:100,now:()=>clock.now});
      const {host,actors}=makeRoom(service,'heading_for_new_shores');
      queueGoldClaims(service.roomFor(host.code).game,[{playerId:actors[0].seatId,amount:1}]);
      assert.equal(service.observe(host.code,actors[0].token).gameState.pendingChoice.kind,'goldResource');
      if(reason==='manual')assert.equal(issue(service,host,host,'closeRoom').success,true);
      else clock.now+=100;
      const watched=service.watch(host.code);
      assert.equal(watched.status,'closed');assert.equal(watched.closeReason,reason);
      assert.equal(watched.gameState.pendingChoice,null);
      assert.deepEqual(watched.legalActions,[]);
      const replay=service.replay(host.replayId,{perspective:'omniscient'});
      assert.equal(replay.success,true);assert.equal(replay.state.gameState.pendingChoice,null);
      assert.equal(replay.recording.rulesVersion,'catan-seafarers-2025-v1');
      assert.equal(store.getRecording(host.replayId).endedAt,clock.now);
    } finally {store.close();}
  }
});
